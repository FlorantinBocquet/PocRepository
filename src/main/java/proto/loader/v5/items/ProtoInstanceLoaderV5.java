...
import proto.loader.v5.items.configuration.EmptyConfigurationFileLoaderV5;

public class ProtoInstanceLoaderV5 {
  private final ProtoClassLoaderV5 loader;
  private final ConfigurationFileLoaderV5 conf;
  ...
  public ProtoInstanceLoaderV5() {
    this(EmptyConfigurationFileLoaderV5.getInstance());
  }

  public ProtoInstanceLoaderV5(final ConfigurationFileLoaderV5 conf) {
    loader = new ProtoClassLoaderV5();
    this.conf = conf;
  }
  ...
  public void create() throws IllegalStateException {
    if (objects == null) {
      ...
      List<ElementV5> toLoad = new ArrayList<>(loader.retrieveLoadable().values());

      // If an element fails to load because of missing parameters, it will be stored in this list.
      List<ElementV5> notLoaded = new ArrayList<>();
      ...
      do {
        ...
        do {
          final ElementV5 current = toLoad.remove(toLoad.size() - 1);
  ...
  private Object loadItem(
      final ElementV5 current,
      final Map<String, Object> loadedForMethods
  ) throws MissingParamException {
    if (current.annotationClass() == ProtoGenerableV5.class) {
    ...
    } else if (current.annotationClass() == ProtoLoadableV5.class) {
      return loadForLoadable(current);
    } else if (current.annotationClass() == ProtoConfigV5.class) {
  ...
  private void assertExactlyOneConstructor(final ElementV5 element) {
  ...
  private Object loadForConfig(final ElementV5 current) {
    final String rootPath = current.clazz().getAnnotation(ProtoConfigV5.class).value();
  ...
  private Object[] loadArgs(final Parameter[] parameters) throws MissingParamException {
    ...
    for (int i = 0; i < parameters.length; i++) {
      ...
      final ProtoValueV5 value = parameter.getAnnotation(ProtoValueV5.class);
  ...
  public Object get(@Nullable final String paramName, @Nullable final Class<?> paramType) {
    try {
      ...
      try {
        ...
        return objects.get(UtilsV5.getQualifierFromClass(paramType));
      } catch (Exception e2) {
        return null;
      }
    }

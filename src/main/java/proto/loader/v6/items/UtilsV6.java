...
public class UtilsV6 {
  public static String getQualifierFromClass(final Class<?> clazz) {
    return Optional.ofNullable(clazz.getAnnotation(ProtoQualifierV6.class))
        .map(ProtoQualifierV6::value)
        .orElse(removeKtSuffix(clazz.getSimpleName()));
  }

  public static String getQualifierFromMethod(final Method method) {
    return Optional.ofNullable(method.getAnnotation(ProtoQualifierV6.class))
        .map(ProtoQualifierV6::value)
